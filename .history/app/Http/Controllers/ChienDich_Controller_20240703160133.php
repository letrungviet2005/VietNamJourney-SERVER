...
class ChienDich_Controller extends Controller
{
    ...
    public function getCampaignsByProvince(Request $request)
    {
        ...
        try {
            // Truy vấn các chiến dịch theo tên tỉnh và ngày trong khoảng thời gian hợp lệ
            ...
            // Xử lý JSON và tạo URL công khai cho hình ảnh
            $campaigns->transform(function ($campaign) {
                // Giải mã JSON cho các trường
                $campaign->timeline = json_decode($campaign->timeline, true);
                $campaign->infoContact = json_decode($campaign->infoContact, true);
                $campaign->infoOrganization = json_decode($campaign->infoOrganization, true);

                // Tạo URL công khai cho hình ảnh
                if (Storage::exists($campaign->image)) {
                    $campaign->image = Storage::url($campaign->image);
                } else {
                    $campaign->image = null; // Hoặc có thể đặt một đường dẫn mặc định nếu hình ảnh không tồn tại
                }
